...
const ConfirmModal: React.FC<ConfirmModalProps> = ({isOpen, onClose, onAfterClose, action, toggleModal, data}) =>
{
                        ...
                        <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div className="sm:flex sm:items-start">
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        ...
                                        <div className="mt-2">
                                            {!response?.status && <p className="text-red-700">{response?.message}</p>}
                            ...
                            <div className="bg-gray-50 px-4 py-3 flex justify-end sm:px-6">
                                { action == "delete" && !response?.status &&
                                    <>
                                        <button onClick={handleClose} type="button" disabled={progress?true:false} className="mt-3 min-w-20 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-300 ring-inset hover:bg-gray-50 sm:mt-0 sm:w-auto focus:ring-4 focus:ring-gray-500/20">Cancel</button>
